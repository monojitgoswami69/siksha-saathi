...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setError(res.error || 'Invalid administrator or faculty credentials');
    }
    setSubmitting(false);
  };

  // Google OAuth 2.0 (dashboard scope)
  const handleGoogleSignIn = () => {
    ...
    if (typeof window !== 'undefined' && (window as any).google?.accounts?.oauth2) {
      ...
      const authUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${encodeURIComponent(
        ...
        redirectUri
      )}&response_type=token&scope=openid%20email%20profile&prompt=select_account&state=dashboard`;
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-white relative font-body text-slate-800">
      ...
      <nav className="absolute top-0 left-0 w-full flex justify-between items-center p-4 px-6 md:px-10 z-50">
        <div className="flex items-center gap-2 text-slate-600 transition-colors">
          ...
          <span className="text-[11px] font-semibold text-[#0d47a1] bg-blue-50 border border-blue-200/60 px-2 py-0.5 rounded-md ml-1">
            Faculty & Admin
          </span>
        </div>
        <Link
      ...
      <div className="flex-1 flex flex-col justify-center px-8 md:px-16 lg:px-24 pt-24 pb-12 w-full lg:max-w-[calc(100%-480px)] xl:max-w-[calc(100%-520px)]">
        <div className="max-w-3xl">
          <h1 className="text-5xl lg:text-[4rem] font-medium leading-[1.1] tracking-tight text-slate-800 mb-6 font-headline">
            Instruct with <span className="text-[#1a559e] font-semibold">AI Precision</span>,
            <br />
            Empower with{' '}
          ...
          <p className="text-slate-500 text-lg md:text-xl max-w-2xl leading-relaxed mb-16">
            Empowering educators with AI-guided syllabus management, real-time student doubt tracking, and institutional learning intelligence.
          </p>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="p-6 rounded-2xl border border-slate-100 bg-white shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
              <h3 className="font-semibold text-slate-800 mb-2">Curriculum Alignment</h3>
              <p className="text-slate-500 text-sm leading-relaxed">
                Map departmental syllabi, modules, and sections. Ensure AI tutoring strictly adheres to institutional course scope.
              </p>
            </div>
            <div className="p-6 rounded-2xl border border-slate-100 bg-white shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
              <h3 className="font-semibold text-slate-800 mb-2">Smart OCR Ingestion</h3>
              <p className="text-slate-500 text-sm leading-relaxed">
                Upload lecture slides, textbooks, and notes with automated chunking, multilingual E5 embeddings, and pgvector storage.
              </p>
            </div>
            <div className="p-6 rounded-2xl border border-slate-100 bg-white shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
              <h3 className="font-semibold text-slate-800 mb-2">Actionable Analytics</h3>
              <p className="text-slate-500 text-sm leading-relaxed">
                Track student query density, pinpoint conceptual friction points, and identify at-risk learners before examinations.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="w-full md:absolute md:right-0 md:top-0 md:h-full md:w-[480px] xl:w-[520px] bg-[#e6ecef] flex flex-col justify-center items-center p-6 lg:p-12 min-h-screen md:min-h-0">
        <div className="w-full max-w-[420px] bg-white rounded-[2rem] p-8 lg:p-10 shadow-xl shadow-slate-200/50">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-slate-800 mb-1 font-headline">Faculty Portal</h2>
            <p className="text-slate-500 text-sm">Sign in to access your educator workspace</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-slate-500 font-medium ml-1">Institutional Email</label>
              <input
                type="email"
                value={email}
            ...
            <div className="bg-[#f8fafc] border border-slate-100 rounded-xl p-3 flex items-center justify-center gap-2 text-xs text-slate-500 mt-1">
              ...
              <span>Restricted to authorized faculty & administrators.</span>
            </div>
          </form>
        </div>
      </div>
