...
export async function GET(req: NextRequest) {
  try {
    ...
    // 1. Total queries (query_logs, scoped)
    const sc = dashboardScopeClause(
      { stream: 'q.stream', semester: 'q.semester', section: 'q.section', subject: 'q.subject' },
      scope,
      1
    );
    const totalQRes = await query(`SELECT COUNT(*)::int as total FROM query_logs q WHERE 1=1${sc.sql};`, sc.params);
    const totalQueries = totalQRes.rows[0]?.total || 0;

    // Total students — scoped by stream/sem/section (no subject column on student_users)
    const ssc = dashboardScopeClause(
      { stream: 's.stream', semester: 's.sem', section: 's.section' },
      scope,
      1
    );
    const totalSRes = await query(`SELECT COUNT(*)::int as total FROM student_users s WHERE 1=1${ssc.sql};`, ssc.params);
    ...
    // 3. Weak domains — per subject from query_citations (distinct queries)
    ...
    // 4. Weekly trend (query_logs scoped)
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      // scope params start at $2 (1 = date)
